...
class MailMergeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Mail Merge to PDF Tool v2.1")
        ...
        self.selected_records = set()  # Track selected records
    ...
    def _save_settings(self):
        ...
        try:
            self.settings.update({
                ...
                "process_selected_only": self.process_selected_only_var.get()
            })
    ...
    def _monitor_queues(self):
        """Monitor both message and log queues for updates"""
        ...
        try:
            while True:
                ...
                elif message_type == 'progress_text':
                    self.progress_text_var.set(message['text'])
        ...
        # Schedule next check
        self.root.after(100, self._monitor_queues)
    ...
    def _create_ui(self):
        ...
        try:
            ...
            # Setup left panel with notebook
    ...
    def _setup_left_panel(self, parent):
        """Setup the left panel with configuration tabs"""
    ...
    def _setup_right_panel(self, parent):
        """Setup the right panel with logs and progress"""
        # Progress section
        ...
        progress_label = ttk.Label(progress_info_frame, textvariable=self.progress_text_var)
        progress_label.pack(side=tk.RIGHT, padx=(5, 0))
        
        # Control buttons
        control_frame = ttk.Frame(progress_frame)
        control_frame.pack(fill=tk.X, padx=5, pady=5)
        
        self.run_button = ttk.Button(control_frame, text="🚀 Run Mail Merge", 
        ...
        ttk.Button(control_frame, text="Clear Logs", 
                  command=self._clear_logs).pack(side=tk.RIGHT, padx=5)
        
        # Live logs section
        logs_frame = ttk.LabelFrame(parent, text="Live Logs")
    ...
    def _create_menu(self):
        ...
        tools_menu.add_command(label="Select All Records", command=self._select_all_records)
        tools_menu.add_command(label="Deselect All Records", command=self._deselect_all_records)
    ...
    def _create_status_bar(self):
        ...
        self.selection_var.set("No data loaded")
        selection_label = ttk.Label(status_frame, textvariable=self.selection_var, anchor=tk.E)
        selection_label.pack(side=tk.RIGHT, padx=10)
    ...
    def _update_selection_status(self):
        ...
        if self.preview_data is None:
            ...
            self.selection_var.set(f"Selected: {selected}/{total}")
    ...
    def _setup_config_tab(self, parent):
        ...
        try:
            ...
            # Processing options
            processing_frame = ttk.LabelFrame(scrollable_frame, text="Processing Options")
            processing_frame.pack(fill=tk.X, padx=10, pady=10)
            
            self.process_selected_only_var = tk.BooleanVar(value=self.settings.get("process_selected_only", False))
            ...
            ttk.Checkbutton(processing_frame, text="Process selected records only", 
                          variable=self.process_selected_only_var).pack(anchor=tk.W, pady=2, padx=5)
            ttk.Checkbutton(processing_frame, text="Skip records with errors (continue processing)", 
                          variable=self.skip_errors_var).pack(anchor=tk.W, pady=2, padx=5)
            ttk.Checkbutton(processing_frame, text="Open output folder when complete", 
                          variable=self.open_output_var).pack(anchor=tk.W, pady=2, padx=5)
            ttk.Checkbutton(processing_frame, text="Backup existing files", 
                          variable=self.backup_existing_var).pack(anchor=tk.W, pady=2, padx=5)
            
            # Filename pattern
            pattern_frame = ttk.Frame(processing_frame)
    ...
    def _setup_preview_tab(self, parent):
        """Configure the Preview tab with enhanced selection features"""
        try:
            # Control frame
            ...
            ttk.Label(info_frame, text="Records:").pack(side=tk.LEFT)
            self.record_count_var = tk.StringVar(value="0")
            ttk.Label(info_frame, textvariable=self.record_count_var, font=("TkDefaultFont", 9, "bold")).pack(side=tk.LEFT, padx=5)
            
            # Right side - controls
            button_frame = ttk.Frame(control_frame)
            button_frame.pack(side=tk.RIGHT)
            
            ttk.Button(button_frame, text="🔄 Refresh", command=self.load_preview).pack(side=tk.LEFT, padx=2)
            ttk.Button(button_frame, text="Select All", command=self._select_all_records).pack(side=tk.LEFT, padx=2)
            ttk.Button(button_frame, text="Deselect All", command=self._deselect_all_records).pack(side=tk.LEFT, padx=2)
            ttk.Button(button_frame, text="Invert Selection", command=self._invert_selection).pack(side=tk.LEFT, padx=2)
            ...
            # Create a Treeview widget for displaying data with checkboxes
            ...
            self.preview_tree.bind('<Double-1>', self._toggle_record_selection)
            self.preview_tree.bind('<Button-1>', self._on_tree_click)
            
            # Help text
            help_label = ttk.Label(parent, text="Double-click rows to select/deselect for processing. Selected rows are highlighted.", 
